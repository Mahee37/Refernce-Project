			...
			<tbody>
				<c:forEach var="order" items="${orders}">
					<tr style="transition: all 0.3s ease;">
						...
						<td>&#8377;${order.price}</td>
						<td>${order.buyerId}</td>
						<td>${order.shippingAddress}<br> ${order.city} - ${order.pincode}<br> Phone: ${order.phoneNumber}</td>
						<td>${order.orderDate}</td>
						<td>${order.status}</td>
						<td>
							<c:if test="${order.status ne 'Delivered' && order.status ne 'Cancelled'}">
								<form method="post" action="${pageContext.request.contextPath}/retailer/orders">
